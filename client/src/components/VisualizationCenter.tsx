...
export default function VisualizationCenter() {
  ...
  return (
    <div className="flex flex-col w-full h-full bg-slate-900 overflow-hidden rounded-xl border border-slate-700 shadow-2xl">
      {/* 头部 */}
      <div className="p-4 border-b border-slate-800 bg-slate-800/50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white font-mono">网络知识可视化中心</h1>
              <p className="text-xs text-slate-400 font-mono tracking-wider uppercase">Interactive Network Concepts</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex-1">
                    ...
                    <Badge variant="secondary" className="bg-slate-800 text-slate-400 text-[10px] font-mono border-none">
                      {module.category}
                    </Badge>
                  </div>
                </div>
                <p className="text-[11px] text-slate-400 mb-3 leading-relaxed">{module.description}</p>
                <div className="flex flex-wrap gap-1">
                  {(module.topics || []).slice(0, 3).map((topic, i) => (
                    <span key={i} className="px-1.5 py-0.5 rounded bg-slate-900/50 text-slate-500 text-[9px] font-mono border border-slate-800">
                      {topic}
                    </span>
                  ))}
                </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-950/30">
          <div className="max-w-4xl mx-auto h-full">
            <div key={selectedModule} className="h-full">
              {renderModule()}
            </div>
          </div>
        </div>
